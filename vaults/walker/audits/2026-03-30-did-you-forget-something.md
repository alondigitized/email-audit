---
slug: 2026-03-30-did-you-forget-something
type: email
date: 2026-03-30
persona: walker
score: "6/10"
...
- Sender identification matching the `From:` header (`hello@msgs.skechers.com`)
...
- DKIM: `d=msgs.skechers.com` or `d=skechers.com` with valid signature
- SPF: `msgs.skechers.com` included in Skechers' SPF record
- DMARC: policy on `skechers.com` should be at least `p=quarantine`
...
- `utm_medium=email`
- `utm_campaign` matches the send (e.g., `abandoned_cart` or campaign ID)
- `utm_content` differentiates CTA placement if multiple links point to the same destination
