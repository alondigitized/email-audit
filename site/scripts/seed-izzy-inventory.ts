/**
 * Seed Izzy Inventory — the bargain hunter of the Inventory family
 ...
 *
 * Usage:
 *   cd site && npx tsx --env-file=.env.local scripts/seed-izzy-inventory.ts
 */
...
async function main() {
  ...
  const profile = personaProfileSchema.parse({
    ...
    color: "#c026d3",
    notes:
      "Hired secret-shopper persona — bargain-hunter member of the Inventory family (Ivy/Ian/Ida/Ike/Iris). Runs scheduled inventory audits against Skechers girls' sale footwear (gender+productLine refinement), judging markdowns by whether they are actually still buyable.",
    status: "active",
  });
